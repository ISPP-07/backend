...
async def test_generate_backup_controller_success(mock_db, mock_file):
    ...
    service.generate_backup_service = AsyncMock(
        return_value="backup_generated")

    result = await generate_backup_controller(mock_db, "password")

    assert result == "backup_generated"
    service.generate_backup_service.assert_called_once_with(
        mock_db, "password")
...
async def test_generate_backup_controller_error(mock_db, mock_file):
    ...
    assert str(
        exc_info.value.detail) == "Internal Server Error Something went wrong"
    service.generate_backup_service.assert_called_once_with(
        mock_db, "password")
...
async def test_restore_backup_controller_success(mock_db, mock_file):
    ...
    service.restore_backup_service.assert_called_once_with(
        mock_db, "password", mock_file)
...
async def test_restore_backup_controller_error(mock_db, mock_file):
    ...
    service.restore_backup_service = AsyncMock(
        side_effect=Exception("Something went wrong"))
