...
def test_get_patient_details(mongo_db: Database, app_client: TestClient):
    patients = [
        {
            "_id": uuid4(),
            "dossier_number": "1",
    ...
    url_get = f'{settings.API_STR}acat/patient/{patients[0]["_id"]}'
    response = app_client.get(url_get)
    assert response.status_code == 200
    response_data = response.json()
    assert response_data["name"] == patients[0]["name"]
    assert response_data["alias"] == patients[0]["alias"]
