...
@router.get('/', status_code=status.HTTP_200_OK, response_model=list[Intervention])
async def get_interventions(db: DataBaseDep):
    return await controller.get_interventions_controller(db)


@router.get('/{intervention_id}', status_code=status.HTTP_200_OK, response_model=Intervention)
